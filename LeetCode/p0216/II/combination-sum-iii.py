...
# File Name: combination-sum-iii.py
# Author: Ynjxsjmh
# Email: ynjxsjmh@gmail.com
# Created: 2022-06-25 12:44:16
# Last Updated: 
#           By: Ynjxsjmh
# Description: Find all valid combinations of `k` numbers that sum up
...
class Solution(object):
    def combinationSum3(self, k, n):
        ...
        return self.dfs(range(1, 10), n, k, 0, [], [])

    def dfs(self, nums, target, n, idx, combination, combinations):
        '''
        idx 当前选择 nums 中的第几个数
        '''
        if target < 0:
            return combinations[:]
        ...
        for i in range(idx, len(nums)):
            res.extend(self.dfs(nums, target-nums[i], n, i+1, combination+[nums[i]], combinations[:]))
