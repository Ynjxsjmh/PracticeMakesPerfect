...
# Created: 2022-07-09 01:18:28
# Last Updated: 
#           By: Ynjxsjmh
# Description: You are given an array of non-overlapping intervals
# `intervals` where `intervals[i] = [starti, endi]` represent the
# start and the end of the `ith` interval and `intervals` is sorted in
...
class Solution(object):
    def insert(self, intervals, newInterval):
        ...
        for i in range(len(intervals)):
            ...
            elif pre_interval[0] > cur_interval[1]:
                res = res[:-1] + [cur_interval, res[-1]]
            else:
                res = res[:-1]
                res.append([
                    min(pre_interval[0], cur_interval[0]),
                    max(pre_interval[1], cur_interval[1])
                ])
